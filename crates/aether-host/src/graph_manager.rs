...
use aether_nodes::{
    chorus::Chorus, compressor::Compressor, delay::DelayLine, envelope::AdsrEnvelope,
    filter::StateVariableFilter, formant::FormantFilter, gain::Gain, granular::Granular,
    karplus_strong::KarplusStrong, lfo::Lfo, mixer::Mixer, moog_ladder::MoogLadder,
    oscillator::Oscillator, record::RecordNode, reverb::Reverb, scope::ScopeNode,
...
pub struct GraphManager {
    ...
    pub timbre_slots: HashMap<
        NodeId,
        std::sync::Arc<std::sync::Mutex<Option<aether_timbre::analysis::TimbreProfile>>>,
    >,
}
...
impl GraphManager {
        ...
        match intent {
            Intent::AddNode { node_type } => {
                ...
                let node_id = match scheduler.graph.add_node(node) {
                    ...
                    None => {
                        return Response::Error {
                            message: "Graph full".into(),
                        }
                    }
                };
                init_default_params(scheduler, node_id, &node_type);
                match extra {
                    NodeExtra::MidiQueue(q) => {
                        self.midi_queues.lock().unwrap().insert(node_id, q);
                    }
                    NodeExtra::ScopeConsumer(consumer) => {
                        self.scope_consumers.insert(node_id, consumer);
                    }
                    NodeExtra::SamplerSlot {
                        midi_queue,
                        instrument_slot,
                    } => {
                    ...
                    NodeExtra::TimbreSlot(slot) => {
                        self.timbre_slots.insert(node_id, slot);
                    }
                    _ => {}
                }
            ...
            Intent::RemoveNode {
                ...
                // Push undo entry
                let entry = UndoEntry {
                    forward: StructuralIntent::RemoveNode {
                        node_id,
                        generation,
                        node_type: node_type.clone(),
                    },
                    inverse: StructuralIntent::AddNode {
                        node_type,
                        created_id: id,
                    },
                };
            ...
            Intent::Connect {
                ...
                let inverse = if let Some(ps) = prev_src {
                    StructuralIntent::Connect {
                        src_id: ps.index,
                        src_gen: ps.generation,
                        dst_id,
                        dst_gen,
                        slot,
                    }
                } else {
                    StructuralIntent::Disconnect {
                        dst_id,
                        dst_gen,
                        slot,
                        prev_src_id: src_id,
                        prev_src_gen: src_gen,
                    }
                };
                let entry = UndoEntry {
                    forward: StructuralIntent::Connect {
                        src_id,
                        src_gen,
                        dst_id,
                        dst_gen,
            ...
            Intent::Disconnect {
                ...
                if let Some(ps) = prev_src {
                    let entry = UndoEntry {
                        forward: StructuralIntent::Disconnect {
                            dst_id,
                            dst_gen,
                            slot,
                            prev_src_id: ps.index,
                            prev_src_gen: ps.generation,
                        },
                        inverse: StructuralIntent::Connect {
                            src_id: ps.index,
                            src_gen: ps.generation,
                            dst_id,
                            dst_gen,
            ...
            Intent::UpdateParam {
                ...
                Response::Ack {
                    command: "update_param".into(),
                }
            }

            Intent::SetOutputNode {
                node_id,
                generation,
            } => {
                let id = NodeId {
            ...
            Intent::ClearGraph => {
                ...
                // Push undo entry
                let entry = UndoEntry {
                    forward: StructuralIntent::ClearGraph {
                        snapshot_before: snapshot_before.clone(),
                    },
                    inverse: StructuralIntent::LoadPatch {
                        patch: snapshot_before,
                    },
                };
                self.undo_stack.push(entry);
                self.redo_stack.clear();
            ...
            Intent::LoadPatch { patch } => {
                ...
                for pnode in &patch.nodes {
                    let (node, extra) = match make_node(&pnode.node_type, scheduler.sample_rate) {
                        ...
                        None => {
                            return Response::Error {
                                message: format!("Unknown node type: {}", pnode.node_type),
                            }
                        }
                    };
                    let node_id = match scheduler.graph.add_node(node) {
                ...
                for conn in &patch.connections {
                    let src = match local_id_map.get(&conn.from) {
                        Some(id) => *id,
                        None => {
                            return Response::Error {
                                message: format!("Unknown node id: {}", conn.from),
                            }
                        }
                    };
                    let dst = match local_id_map.get(&conn.to) {
                        Some(id) => *id,
                        None => {
                            return Response::Error {
                                message: format!("Unknown node id: {}", conn.to),
                            }
                        }
                    };
                ...
                // Push undo entry
                let entry = UndoEntry {
                    forward: StructuralIntent::LoadPatch {
                        patch: patch.clone(),
                    },
                    inverse: StructuralIntent::LoadPatch {
                        patch: snapshot_before,
            ...
            Intent::MidiListPorts => Response::Error {
                message: "MidiListPorts must be handled in ws_server".into(),
            },
            Intent::MidiConnect { .. } => Response::Error {
                message: "MidiConnect must be handled in ws_server".into(),
            },
            ...
            Intent::InjectMidi {
                ...
                if let Some(target_id) = self.midi_routes.get(&channel) {
                    ...
                    for q in queues.values() {
                        q.lock().unwrap().push(event.clone());
                    }
                }
                Response::Ack {
                    command: "inject_midi".into(),
                }
            }
            ...
            Intent::RemoveMidiRoute { channel } => {
                ...
                Response::Ack {
                    command: "remove_midi_route".into(),
                }
            }

            Intent::LoadInstrument {
                node_id,
                generation,
                instrument_json,
            } => {
                ...
                let slot = match self.instrument_slots.get(&id) {
                    ...
                    None => {
                        return Response::Error {
                            message: format!(
                                "No SamplerNode found with id {node_id} — add a SamplerNode first"
                            ),
                        }
                    }
                };
                ...
                for zone in &instrument.zones {
                    ...
                    if path.exists() {
                        match aether_sampler::buffer::SampleBuffer::load_wav(&path) {
                            ...
                            Err(e) => eprintln!(
                                "LoadInstrument: failed to load '{}': {e}",
                                path.display()
                            ),
                        }
                    } else {
                        eprintln!(
                            "LoadInstrument: zone '{}' file not found: {}",
                            zone.id,
                            path.display()
                        );
                    }
                ...
                Response::Ack {
                    command: "load_instrument".into(),
                }
            }
            Intent::SetModulation {
                src_node_id,
                src_gen,
                dst_node_id,
                dst_gen,
                ...
                self.mod_connections.retain(|c| {
                    !(c.src_node_id == src_node_id
                        && c.dst_node_id == dst_node_id
                        && c.dst_param_index == dst_param_index)
                });
                ...
                Response::Ack {
                    command: "set_modulation".into(),
                }
            }

            Intent::RemoveModulation {
                src_node_id,
                dst_node_id,
                dst_param_index,
            } => {
                ...
                Response::Ack {
                    command: "remove_modulation".into(),
                }
            }
            Intent::AnalyzeTimbre { instrument_json } => {
                let instrument: aether_sampler::instrument::SamplerInstrument =
                    match serde_json::from_str(&instrument_json) {
                        ...
                        Err(e) => {
                            return Response::Error {
                                message: format!("AnalyzeTimbre: bad JSON: {e}"),
                            }
                        }
                    };
                ...
                if profile.envelopes.is_empty() {
                    return Response::Error {
                        message: "AnalyzeTimbre: no valid audio zones found".into(),
                    };
                }
                ...
                Response::Ack {
                    command: "analyze_timbre".into(),
                }
            }

            Intent::ApplyTimbreTransfer {
                timbre_transfer_node_id,
                profile_name,
            } => {
                ...
                match slot {
                    ...
                    Some(slot) => {
                        ...
                        if has_profile {
                            Response::Ack {
                                command: "apply_timbre_transfer".into(),
                            }
                        } else {
                            Response::Error {
                                message: "No timbre profile loaded — run AnalyzeTimbre first"
                                    .into(),
                            }
                        }
                    }
            ...
            Intent::StopRecording => match self.recording_state.take() {
                None => Response::Ack {
                    command: "stop_noop".into(),
                },
                Some(state) => {
            ...
            Intent::Undo => match self.undo_stack.pop() {
                None => Response::Ack {
                    command: "undo_noop".into(),
                },
                Some(entry) => {
            ...
            Intent::Redo => match self.redo_stack.pop() {
                None => Response::Ack {
                    command: "redo_noop".into(),
                },
                Some(entry) => {
            ...
            Intent::GetCpuUsage => {
                ...
                let entries: Vec<NodeCpuEntry> = scheduler
                    .graph
                    .execution_order
                    .iter()
                    .filter_map(|id| {
    ...
    pub fn apply_modulation(&self, scheduler: &mut Scheduler) {
        for conn in &self.mod_connections {
            let src_id = NodeId {
                index: conn.src_node_id,
                generation: conn.src_gen,
            };
            let dst_id = NodeId {
                index: conn.dst_node_id,
                generation: conn.dst_gen,
            };
    ...
    pub fn snapshot(&self, scheduler: &Scheduler) -> Response {
        let nodes: Vec<NodeSnapshot> = scheduler
            .graph
            .execution_order
            .iter()
            .filter_map(|id| {
                scheduler.graph.arena.get(*id).map(|r| NodeSnapshot {
                    id: id.index,
                    generation: id.generation,
                    node_type: r.processor.type_name().to_string(),
                    params: r.params.params[..r.params.count]
                        .iter()
                        .map(|p| p.current)
                        .collect(),
                })
...
fn make_node(
    node_type: &str,
    _sample_rate: f32,
) -> Option<(Box<dyn aether_core::node::DspNode>, NodeExtra)> {
    match node_type {
        ...
        "RecordNode" => {
            ...
            Some((
                Box::new(RecordNode::new(producer)),
                NodeExtra::RecordConsumer(consumer),
            ))
        }
        "ScopeNode" => {
            let (producer, consumer) = HeapRb::<f32>::new(512).split();
            Some((
                Box::new(ScopeNode::new(producer)),
                NodeExtra::ScopeConsumer(consumer),
            ))
        }
...
fn export_clap(output_path: String) -> Response {
    ...
    let status = Command::new("cargo")
        .args(["build", "-p", "aether-plugin", "--release"])
        .env("AETHER_INSTRUMENT_JSON", instrument_json)
    ...
    if !status.success() {
        return Response::Error {
            message: format!("cargo build failed: {:?}", status.code()),
        };
    }
    let clap_src = workspace_root
        .join("target")
        .join("release")
        .join("aether_plugin.clap");
    if !clap_src.exists() {
        return Response::Error {
            message: format!(
                "Build succeeded but .clap not found at: {}",
                clap_src.display()
            ),
        };
    }
    ...
    if let Err(e) = std::fs::copy(&clap_src, dest) {
        return Response::Error {
            message: format!("Failed to copy .clap: {e}"),
        };
    }
    let size_bytes = std::fs::metadata(dest).map(|m| m.len()).unwrap_or(0);
    Response::ClapExported {
        path: output_path,
        size_bytes,
    }
}
